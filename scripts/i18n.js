/**
 * i18n DevCoveragePlansEngine (misma dinámica ES/EN que Tuk Tuk / Job Copilot).
 * i18next + p2l-locale + catálogos embebidos.
 */
(function () {
  ...
  async function setLocale(code, opts = {}) {
    ...
    if (typeof window.onDevCoverageLocaleChange === 'function') {
      window.onDevCoverageLocaleChange(lang, { skipScroll: !!opts.skipScroll })
    }
  }

  async function initI18n() {
    if (typeof i18next === 'undefined') {
      console.error('[DevCoveragePlansEngine] i18next no cargó')
      return
    }

    const resources = window.DEV_COVERAGE_I18N_RESOURCES
    if (!resources || !resources.es || !resources.en) {
      console.error('[DevCoveragePlansEngine] Falta scripts/locales-embed.js')
      return
    }
    ...
    if (typeof window.onDevCoverageI18nReady === 'function') {
      window.onDevCoverageI18nReady(initial)
    }
  }
